...
# 유틸리티 클래스 임포트
from utils import MetricTracker, GPUMonitor, RankAwareLogger, PathManager, WeightedCrossEntropyLoss, create_weighted_loss_fn
...
def evaluate_model_comprehensive(model_engine, eval_dataloader, local_rank=0, metric_tracker=None, prefix=""):
    ...
    with torch.no_grad():
        for batch in tqdm(eval_dataloader, desc="Evaluating", disable=local_rank != 0):
            ...
            # Calculate loss using weighted cross entropy
...
def main():
    ...
    parser.add_argument("--config", type=str, default="conf/base/deepspeed.yml")
    ...
    # 가중 손실 함수 생성
    loss_fn = None
    if dataset_metadata and 'class_weights' in dataset_metadata:
        device = torch.device(f"cuda:{local_rank}")
        loss_fn = create_weighted_loss_fn(
            dataset_metadata=dataset_metadata,
            device=device,
            label_smoothing=config.training.get('label_smoothing', 0.0)
        )
        if local_rank == 0:
            print(f"✅ 가중 손실 함수 생성 완료")
    else:
        if local_rank == 0:
            print(f"⚠️ 클래스 가중치를 찾을 수 없습니다. 기본 손실 함수를 사용합니다.")
